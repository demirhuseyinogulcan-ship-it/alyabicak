...
export default async function BlogPostPage({ params }: PageProps) {
  ...
  const breadcrumbItems = [
    ...
    { name: post.category.name, url: `/${locale}/newsletter/kategori/${post.category.slug}` },
    { name: post.title },
  ]

  // Schema.org Article
  const articleSchema = generateArticleSchema(post, locale as SupportedLocale)
            ...
            <div className="flex flex-wrap items-center gap-4 md:gap-6 text-steel-300">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-steel-400">{post.author.title}</p>
                </div>
              </div>
        ...
        <div className="container mx-auto px-4 py-12">
          <div className="grid lg:grid-cols-12 gap-12">
            ...
            <main className="lg:col-span-8">
              ...
              {/* Excerpt */}
              <p className="text-xl text-steel-700 leading-relaxed mb-8 font-medium">
                {post.excerpt}
              </p>

              {/* Article Content */}
              <div 
                className="prose prose-lg prose-steel max-w-none
                  prose-headings:font-bold prose-headings:text-steel-900
                  prose-h2:text-2xl prose-h2:mt-12 prose-h2:mb-4
                  prose-h3:text-xl prose-h3:mt-8 prose-h3:mb-3
                  prose-p:text-steel-700 prose-p:leading-relaxed
                  prose-a:text-primary-600 prose-a:no-underline hover:prose-a:underline
                  prose-strong:text-steel-900
                  prose-ul:my-6 prose-li:my-2
                  prose-img:rounded-xl prose-img:shadow-lg
                  prose-figcaption:text-center prose-figcaption:text-steel-500 prose-figcaption:text-sm"
            ...
            <aside className="lg:col-span-4">
              <div className="sticky top-24 space-y-8">
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm">
                  <h3 className="text-lg font-semibold text-steel-900 mb-4">
                    {dict.blog.categories.all}
                  </h3>
                  <div className="space-y-2">
                    {blogService.getAllPosts().reduce((categories, p) => {
                      ...
                      return categories
                    }, [] as Array<{ id: string; name: string; slug: string; count: number }>).map((category) => (
                      <Link
                        key={category.id}
                ...
                <div className="bg-gradient-to-br from-primary-600 to-primary-700 rounded-2xl p-6 text-white">
                  ...
                  <p className="text-primary-100 text-sm mb-4">
                    {locale === 'tr' 
                      ? 'Teknik konularda size yardımcı olmaktan mutluluk duyarız.'
                      : 'We\'re happy to help you with technical questions.'}
                  </p>
                  <Link
                    href={`/${locale}/contact`}
