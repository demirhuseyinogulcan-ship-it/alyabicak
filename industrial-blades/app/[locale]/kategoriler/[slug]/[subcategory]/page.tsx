...
export async function generateMetadata({ params }: PageProps) {
  ...
  return generateSeoMetadata({
    ...
    url: `https://www.alyabicak.com/${locale}/kategoriler/${category.slug}/${subcategory.slug}`,
  })
}
...
export default async function SubcategoryPage({ params }: PageProps) {
  ...
  const breadcrumbSchema = generateBreadcrumbSchema([
    ...
    { name: dict.nav.categories, url: `https://www.alyabicak.com/${locale}/kategoriler` },
    { name: category.name, url: `https://www.alyabicak.com/${locale}/kategoriler/${category.slug}` },
    { name: subcategory.name, url: `https://www.alyabicak.com/${locale}/kategoriler/${category.slug}/${subcategory.slug}` },
  ])
        ...
        <PageHeader
          ...
          breadcrumb={
            <nav className="text-sm text-steel-300">
              ...
              <Link href={`/${locale}/kategoriler`} className="hover:text-white">{dict.nav.categories}</Link>
              <span className="mx-2">/</span>
              <Link href={`/${locale}/kategoriler/${category.slug}`} className="hover:text-white">{category.name}</Link>
              <span className="mx-2">/</span>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
              {category.subcategories
                ...
                .map((sub) => (
                  <Link
                    key={sub.id}
                    href={`/${locale}/kategoriler/${category.slug}/${sub.slug}`}
