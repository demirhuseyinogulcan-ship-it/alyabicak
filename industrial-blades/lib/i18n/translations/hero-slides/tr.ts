/**
 * Hero Slides - Türkçe Çevirileri
 * 
 * Bu dosya hero slider verilerinin Türkçe çevirilerini içerir.
 * Yeni dil eklemek için bu dosyayı kopyalayıp çevirin.
 */

export const heroSlidesTr = {
  'slide-1': {
    title: 'Alya Kalite Standartında Endüstriyel Bıçaklar',
    subtitle: 'Dünya Çapında Tanınan Kalite',
    description: 'Üstün çelik kalitesi ile uzun ömürlü ve keskin performans',
    ctaText: 'Ürünleri İncele',
    imageAlt: 'Alya kalite standartında endüstriyel bıçaklar - sanayi tipi kesici bıçaklar, dilme jiletleri, makine bıçakları',
  },
  'slide-2': {
    title: 'Özel Tasarım Bıçaklar',
    subtitle: 'İhtiyacınıza Özel Çözümler',
    description: 'İhtiyacınıza özel üretim, mükemmel kesim kalitesi',
    ctaText: 'Ücretsiz Danışmanlık Alın',
    imageAlt: 'Özel tasarım endüstriyel bıçaklar - ihtiyaca özel kesici takımlar, profesyonel sanayi bıçakları',
  },
  'slide-3': {
    title: 'Profesyonel Danışmanlık',
    subtitle: 'Uzman Teknik Destek',
    description: 'Doğru malzeme seçimi için uzman desteği',
    ctaText: 'Ücretsiz Danışmanlık Alın',
    imageAlt: 'Endüstriyel bıçak danışmanlığı - teknik destek, malzeme seçimi, kesici takım uzmanı',
  },
  'slide-4': {
    title: 'Kesimin En Güvenilir Hali',
    subtitle: 'İş Güvenliği & El Bıçakları',
    description: 'Kesim bitiminde otomatik geri çekilebilen mekanizmasıyla iş kazalarını engeller',
    ctaText: 'Ürünleri İncele',
    imageAlt: 'Güvenlik bıçakları - otomatik geri çekilebilir el bıçağı, iş güvenliği kesici, squeeze knife',
  },
  'slide-5': {
    title: 'Vakum Paketleme Makine Bıçakları',
    subtitle: 'Gıda Güvenliği & Hijyen Standartları',
    description: 'Yıldız, zımba, üçgen ve termoform kesim bıçakları - Tüm vakum paketleme makinalarına uyumlu',
    ctaText: 'Ürünleri İncele',
    imageAlt: 'Vakum paketleme bıçakları - vakum makinası bıçağı, termoform kesim bıçağı, yıldız bıçak, zımba bıçak, üçgen bıçak, gıda paketleme bıçakları',
  },
};

export type HeroSlideTranslations = typeof heroSlidesTr;
