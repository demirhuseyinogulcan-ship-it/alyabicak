# Dil Ekleme Çalışma Listesi

## 📊 Genel Bakış
- **Toplam Dil:** 21
- **Toplam Pazar:** 949M konuşucu
- **Mevcut Diller:** TR, EN, AR, RU (4 dil)

---

## 🎯 FAZ 1 - Yüksek Öncelik (İlk 2 Ay)
**Hedef Pazar:** 381M konuşucu

- [ ] **1. Fransızca (fr)** - 290M konuşucu
  - Fransa, Kanada, Belçika, İsviçre, Afrika
  - Tahmini süre: 3 gün
  
- [ ] **2. Azerice (az)** - 10M konuşucu
  - Azerbaycan, İran
  - Tahmini süre: 2 gün
  
- [ ] **3. Özbekçe (uz)** - 35M konuşucu
  - Özbekistan, Afganistan
  - Tahmini süre: 2 gün
  
- [ ] **4. Rumence (ro)** - 24M konuşucu
  - Romanya, Moldova
  - Tahmini süre: 2 gün
  
- [ ] **5. Bulgarca (bg)** - 9M konuşucu
  - Bulgaristan
  - Tahmini süre: 2 gün
  
- [ ] **6. Yunanca (el)** - 13M konuşucu
  - Yunanistan, Kıbrıs
  - Tahmini süre: 2 gün

**Faz 1 Toplam:** 6 dil, 381M pazar, ~15 gün

---

## 🚀 FAZ 2 - Orta Öncelik (3-4. Aylar)
**Hedef Pazar:** 207M konuşucu

- [ ] **7. Kazakça (kk)** - 19M konuşucu
  - Kazakistan
  - Tahmini süre: 2 gün
  
- [ ] **8. Lehçe (pl)** - 45M konuşucu
  - Polonya
  - Tahmini süre: 2 gün
  
- [ ] **9. Farsça (fa)** - 130M konuşucu
  - İran, Afganistan, Tacikistan
  - Tahmini süre: 3 gün
  
- [ ] **10. Kırgızca (ky)** - 7M konuşucu
  - Kırgızistan
  - Tahmini süre: 2 gün
  
- [ ] **11. Türkmence (tk)** - 6M konuşucu
  - Türkmenistan, İran
  - Tahmini süre: 2 gün

**Faz 2 Toplam:** 5 dil, 207M pazar, ~11 gün

---

## 📈 FAZ 3 - Genişleme (5-6. Aylar)
**Hedef Pazar:** 78M konuşucu

- [ ] **12. Ukraynaca (uk)** - 40M konuşucu
  - Ukrayna
  - Tahmini süre: 2 gün
  
- [ ] **13. Sırpça (sr)** - 12M konuşucu
  - Sırbistan, Bosna-Hersek
  - Tahmini süre: 2 gün
  
- [ ] **14. Gürcüce (ka)** - 4M konuşucu
  - Gürcistan
  - Tahmini süre: 2 gün
  
- [ ] **15. İbranice (he)** - 9M konuşucu
  - İsrail
  - Tahmini süre: 2 gün
  
- [ ] **16. Macarca (hu)** - 13M konuşucu
  - Macaristan
  - Tahmini süre: 2 gün

**Faz 3 Toplam:** 5 dil, 78M pazar, ~10 gün

---

## 🌍 FAZ 4 - Niş Pazarlar (7-12. Aylar)
**Hedef Pazar:** 30M konuşucu

- [ ] **17. Arnavutça (sq)** - 7M konuşucu
  - Arnavutluk, Kosova, Makedonya
  - Tahmini süre: 2 gün
  
- [ ] **18. Makedonca (mk)** - 2M konuşucu
  - Kuzey Makedonya
  - Tahmini süre: 2 gün
  
- [ ] **19. Ermenice (hy)** - 7M konuşucu
  - Ermenistan
  - Tahmini süre: 2 gün
  
- [ ] **20. Moğolca (mn)** - 6M konuşucu
  - Moğolistan, Çin
  - Tahmini süre: 2 gün
  
- [ ] **21. Tacikçe (tg)** - 8M konuşucu
  - Tacikistan
  - Tahmini süre: 2 gün

**Faz 4 Toplam:** 5 dil, 30M pazar, ~10 gün

---

## 🌐 GLOBAL SEO STRATEJİSİ - ÖĞRENME YOL HARİTASI

### 1. Technical SEO
- [x] **Hreflang** - Tüm dillerde düzgün çalışıyor
- [ ] **Site Speed Optimization**
  - [ ] Image lazy loading kontrolü
  - [ ] Bundle size analizi
  - [ ] CDN optimizasyonu
  - [ ] Caching stratejisi
- [ ] **Core Web Vitals**
  - [ ] LCP (Largest Contentful Paint) < 2.5s
  - [ ] FID (First Input Delay) < 100ms
  - [ ] CLS (Cumulative Layout Shift) < 0.1
  - [ ] PageSpeed Insights ile test
- [x] **Structured Data** - Organization, Product, LocalBusiness şemaları var

### 2. International SEO
- [ ] **Keyword Research (Dil Bazlı)**
  - [ ] 🇹🇷 Türkçe - "sanayi jileti", "dilme jileti", "endüstriyel bıçak"
  - [ ] 🇬🇧 İngilizce - "industrial blade", "slitting blade", "razor blade"
  - [ ] 🇫🇷 Fransızca - "lame industrielle", "lame de refendage"
  - [ ] 🇷🇺 Rusça - "промышленные лезвия", "ножи для резки"
  - [ ] 🇸🇦 Arapça - "شفرة صناعية", "شفرة تقطيع"
  - [ ] 🇩🇪 Almanca - "Industrieklinge", "Schlitzmesser"
  - [ ] 🇪🇸 İspanyolca - "cuchilla industrial", "cuchilla de corte"
- [ ] **Local Link Building**
  - [ ] Her dil için yerel endüstri dizinleri
  - [ ] Yerel ticaret odaları
  - [ ] Sektör portalları
- [ ] **Content Localization**
  - [ ] Para birimi gösterimi (€, $, ₺, ر.س, ₽)
  - [ ] Yerel telefon numarası
  - [ ] Yerel referanslar/müşteriler
  - [ ] Ülke bazlı teslimat bilgileri

### 3. Conversion Optimization
- [ ] **A/B Testing**
  - [ ] CTA buton renkleri (dil bazlı)
  - [ ] Form uzunluğu
  - [ ] Hero slider içeriği
- [ ] **Heat Maps**
  - [ ] Hotjar veya Microsoft Clarity kurulumu
  - [ ] Tıklama haritaları analizi
  - [ ] Scroll derinliği analizi
- [ ] **User Journey Analysis**
  - [ ] Funnel analizi (Anasayfa → Kategori → Ürün → İletişim)
  - [ ] Çıkış sayfaları analizi
  - [ ] Dönüşüm oranı takibi

### 4. Analytics
- [ ] **Google Analytics 4**
  - [ ] Dil/ülke bazlı segment oluştur
  - [ ] Dönüşüm hedefleri tanımla (form gönderimi, WhatsApp tıklaması)
  - [ ] E-ticaret tracking (ileride)
- [x] **Search Console** - 5 domain bağlı (TR, EN, FR, RU, AR)
- [ ] **Segment Analysis**
  - [ ] Dil bazlı trafik raporu
  - [ ] Ülke bazlı performans
  - [ ] Cihaz bazlı analiz
  - [ ] Haftalık/aylık rapor otomasyonu

### 5. Content Marketing (Her Dil İçin)
- [ ] **Blog/Rehber İçerik**
  - [ ] "Dilme jileti nasıl seçilir?" (TR)
  - [ ] "How to choose slitting blades" (EN)
  - [ ] "Comment choisir les lames de refendage" (FR)
  - [ ] "Как выбрать лезвия для резки" (RU)
  - [ ] "كيفية اختيار شفرات التقطيع" (AR)
- [ ] **Video İçerik**
  - [ ] Ürün tanıtım videoları
  - [ ] Kullanım kılavuzları
  - [ ] YouTube kanalı (çoklu dil altyazı)
- [ ] **Sosyal Medya**
  - [ ] LinkedIn (B2B odaklı)
  - [ ] Sektörel gruplar

---

## 📈 YENİ DİL EKLEMEİ ÖNCELİK SIRASI (Güncel)

| Öncelik | Dil | Pazar Potansiyeli | Rekabet | Öneri |
|---------|-----|-------------------|---------|-------|
| 🔴 1 | 🇩🇪 Almanca | Çok Yüksek | Orta | Endüstriyel dev, yüksek fiyat toleransı |
| 🔴 2 | 🇪🇸 İspanyolca | Yüksek | Düşük | 500M+ konuşurucu, Latin Amerika |
| 🟠 3 | 🇮🇹 İtalyanca | Yüksek | Orta | Güçlü imalat sektörü |
| 🟠 4 | 🇵🇱 Lehçe | Orta | Düşük | Hızlı büyüyen pazar |
| 🟡 5 | 🇳🇱 Hollandaca | Orta | Düşük | Yüksek alım gücü |
| 🟡 6 | 🇵🇹 Portekizce | Yüksek | Düşük | Brezilya pazarı |
| 🟢 7 | 🇯🇵 Japonca | Yüksek | Yüksek | Premium segment |
| 🟢 8 | 🇨🇳 Çince | Çok Yüksek | Çok Yüksek | Zor ama değerli |
| 🔵 9 | 🇰🇷 Korece | Orta | Orta | Teknoloji odaklı |
| 🔵 10 | 🇮🇳 Hintçe | Yüksek | Düşük | Hızlı büyüyen imalat |

---

## 📝 Her Dil İçin Yapılacaklar

### Adım 1: Dictionary Dosyası Oluştur
- [ ] `lib/i18n/dictionaries/[locale].ts` dosyası oluştur
- [ ] Tüm UI metinlerini çevir (navigation, buttons, forms, etc.)

### Adım 2: Veri Çevirileri
- [ ] `lib/i18n/translations/[locale]/` klasörü oluştur
- [ ] `categories.ts` - Kategori isimleri ve açıklamaları
- [ ] `products.ts` - Ürün isimleri ve açıklamaları
- [ ] `hero-slides.ts` - Ana sayfa slider metinleri
- [ ] `spec-values.ts` - Teknik spesifikasyon değerleri (zaten var)

### Adım 3: Locale Config Güncelle
- [ ] `lib/i18n/config.ts` - Yeni locale ekle
- [ ] `middleware.ts` - Locale listesine ekle
- [ ] Font ayarları (Latin/Cyrillic/Arabic/Hebrew alfabeler)

### Adım 4: Test & Build
- [ ] `npm run build` - Build başarılı mı kontrol et
- [ ] Manual test - Sayfaları gözden geçir
- [ ] SEO meta tags kontrol et

---

## 💰 Maliyet Tahmini

| Faz | Dil Sayısı | Süre | Maliyet (AI+İnsan) |
|-----|-----------|------|-------------------|
| Faz 1 | 6 dil | 2 ay | €2,400 |
| Faz 2 | 5 dil | 2 ay | €2,000 |
| Faz 3 | 5 dil | 2 ay | €2,000 |
| Faz 4 | 5 dil | 2 ay | €2,000 |
| **TOPLAM** | **21 dil** | **8 ay** | **€8,400** |

---

## 🎯 Sonuç Beklentisi

- **Toplam Pazar:** 949M konuşucu (mevcut 270M → +679M büyüme)
- **Coğrafi Kapsam:** 100+ ülke
- **Organik Trafik Artışı:** +300-400%
- **Rekabet Avantajı:** Bölgelerde ilk Türk sanayi bıçak üreticisi

---

## 📌 Notlar

- Her dil için ortalama **~2,500 kelime** çeviri gerekiyor
- AI ilk çeviriyi yapacak, insan düzeltecek (hibrit yaklaşım)
- Teknik terimler için glossary hazır (spec-values.ts)
- RTL desteği zaten var (Arabic için)
- Font sistemleri hazır (Latin, Cyrillic, Arabic, Hebrew)

---

**Son Güncelleme:** 5 Şubat 2026
**Hazırlayan:** GitHub Copilot
